...
void test_smp_state_machine(void)
{
	...
	g_assert_cmpint(alice_otr->smp_state, ==, SMPSTATE_EXPECT1);
	...
	smp_msg_1 = otrv4_smp_initiate(alice_otr);
	otrv4_assert(smp_msg_1);
	g_assert_cmpint(alice_otr->smp_state, ==, SMPSTATE_EXPECT2);
	...
	bob_otr->smp_state = SMPSTATE_EXPECT2;

	tlv_t * smp_msg_2 = otrv4_process_smp(bob_otr, smp_msg_1);
	g_assert_cmpint(bob_otr->smp_state, ==, SMPSTATE_EXPECT1);
	...
	g_assert_cmpint(bob_otr->smp_state, ==, SMPSTATE_EXPECT3);
};
